...
public class PausablePlayer {
	...
	public void play() throws JavaLayerException {
		synchronized (playerLock) {
			switch (playerStatus) {
				case NOTSTARTED:
					final Thread t = new Thread(this::playInternal);
					t.setDaemon(true);
		...
		while (playerStatus != FINISHED) {
			try {
				...
				if (positionChangedListener != null) {
					positionChangedListener.position(player.getPosition());
				}
	...
	public BufferedPlayer getInnerPlayer() {
		return player;
	}
}